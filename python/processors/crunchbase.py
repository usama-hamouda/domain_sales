"""Crunchbase search via Google site: operator."""
...
def run(payload: dict) -> dict:
    ...
    driver = get_driver()
    raw = site_google_search(driver, "crunchbase.com", query, max_pages)
    ...
    for r in raw:
        ...
        if mt == "none":
            continue
        classified.append({
            "domain": r.get("domain") or "crunchbase.com",
            "title": r.get("title"),
    ...
    return {
        "ok": True,
        "query": f"site:crunchbase.com {query}",
        "results": classified,
        "raw_count": len(raw),
    }
